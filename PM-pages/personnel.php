...
<body>
    ...
    <div class="page_body">
            ...
            <div id="selections">
                <form action="" method="post">
                    ...
                    <div style="width: 33%;float: left;height: 100%;padding-right:20px;padding-top:5px;text-align:right;">
                        ...
                        Key: <input type="text" name="filter_key" style="width: 114px; margin-bottom: 2px;" value="<?php echo isset($_POST['filter_key']) ? $_POST['filter_key'] : '' ?>">
                        <br>
        ...
        <div class="add_button"><button onclick="openAddUser()" class="add_user_button">Add Person</button></div>
            ...
            if (!$filter_err) {
                ...
                if ($res = mysqli_query($link, $sql)) {
                    ...
                    if (mysqli_num_rows($res) > 0) {
                        ...
                        if ($table_select[1]) {
                            echo "<th class='person_header'>First Name</th>";
                        }
                        if ($table_select[2]) {
                            echo "<th class='person_header'>Last Name</th>";
                        }
                        if ($table_select[3]) {
                            echo "<th class='person_header'>Organization</th>";
                        }
                        if ($table_select[4]) {
                            echo "<th class='person_header'>Paygrade</th>";
                        }
                        if ($table_select[5]) {
                            echo "<th class='person_header' style='width: 200px;'>Email</th>";
                        }
                        if ($table_select[6]) {
                            echo "<th class='person_header' style='width: 90px;'>Start Date</th>";
                        }
                        ...
                        while ($row = mysqli_fetch_array($res)) {
                            ...
                            if ($table_select[0]) {
                                echo "<td class='person_data'>" . $row['user_ID'] . "</td>";
                            }
                            if ($table_select[1]) {
                                echo "<td class='person_data'>" . $row['First_Name'] . "</td>";
                            }
                            if ($table_select[2]) {
                                echo "<td class='person_data'>" . $row['Last_Name'] . "</td>";
                            }
                            if ($table_select[3]) {
                                echo "<td class='person_data'>" . $row['group_name'] . "</td>";
                            }
                            if ($table_select[4]) {
                                echo "<td class='person_data'>" . $row['Pay_Grade'] . "</td>";
                            }
                            if ($table_select[5]) {
                                echo "<td class='person_data' style='width: 200px;'>" . $row['Email'] . "</td>";
                            }
                            if ($table_select[6]) {
                                echo "<td class='person_data' style='width: 90px;'>" . $row['Activation_Date'] . "</td>";
                            }
                            echo "<td class='person_data' style='width: 70px;'><button type='submit' class='trigger' data-modal-trigger='trigger-" . $row['user_ID'] . "'>View Info</button></td>";
                            echo "</tr>";
                        }
                        echo "</table>";
        ...
        <!-- javascript for removal confirmation slide-out -->
        <script>
